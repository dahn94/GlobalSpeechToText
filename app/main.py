...
async def audio_file(file: UploadFile):
    ...
    if extension in ("wav", "mp3", "m4a"):
        ...
        if not os.path.exists('audios_separados'):
            ...
            subprocess.run(['chmod', '-R', '777', 'audios_separados'])

        # criar pasta para textos (se nao existir)
        if not os.path.exists('textos'):
            os.makedirs('textos')
            # chmd 777 para todos os arquivos dentro da pasta textos
            subprocess.run(['chmod', '-R', '777', 'textos'])

        # criar pasta audios (se nao existir)
        if not os.path.exists('audios'):
            os.makedirs('audios')
            subprocess.run(['chmod', '-R', '777', 'audios'])
